...
test('runtime config follows Draft Validate Diff Save version bump Eval eligible flow',async()=>{await withServer(makeRouter(['runtime.read','runtime.config.edit']),async(baseUrl)=>{
  ...
  const secret=await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'AGENT_MODEL',endpointAlias:'candidate',secret:'plaintext'}]});assert.equal(secret.status,400)
  const urlValue=await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'AGENT_MODEL',endpointAlias:'https://model.example?token=secret'}]});assert.equal(urlValue.status,400)
  const created=await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'AGENT_MODEL',adapter:'mock-agent-model-v2',endpointAlias:'agent-model-candidate'}]});assert.equal(created.status,201);const draft=(await created.json()) as {data:{draftId:string;status:string;baseVersion:number;eligibleForPromote:boolean}};assert.equal(draft.data.status,'DRAFT');assert.equal(draft.data.baseVersion,1);assert.equal(draft.data.eligibleForPromote,false)
  const saveTooEarly=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/save`,{});assert.equal(saveTooEarly.status,409)
  const injectedAction=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/validate`,{promote:true});assert.equal(injectedAction.status,400)
  const validated=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/validate`,{});assert.equal(validated.status,200);const validatedBody=await validated.json() as {data:{status:string;diff:Array<{field:string;before:string;after:string}>;validationIssues:unknown[]}};assert.equal(validatedBody.data.status,'VALIDATED');assert.equal(validatedBody.data.validationIssues.length,0);assert.deepEqual(validatedBody.data.diff.map((item)=>item.field),['adapter','endpointAlias'])
  const saved=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/save`,{});assert.equal(saved.status,200);const savedBody=await saved.json() as {data:{status:string;savedVersion:number}};assert.equal(savedBody.data.status,'SAVED');assert.equal(savedBody.data.savedVersion,2)
  ...
  const evaluated=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/eval`,{});assert.equal(evaluated.status,200);const evalBody=await evaluated.json() as {data:{status:string;eligibleForPromote:boolean;evalResult:{status:string;checks:unknown[]}}};assert.equal(evalBody.data.status,'EVALUATED');assert.equal(evalBody.data.evalResult.status,'PASS');assert.equal(evalBody.data.evalResult.checks.length,10);assert.equal(evalBody.data.eligibleForPromote,true)
  const noPromoteRoute=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${draft.data.draftId}/promote`,{});assert.equal(noPromoteRoute.status,404)
})})

test('validation errors and concurrent drafts fail closed',async()=>{await withServer(makeRouter(['runtime.read','runtime.config.edit']),async(baseUrl)=>{
  await selectProject(baseUrl)
  const bad=await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'OPENSEARCH',adapter:'broken-opensearch'}]});const badDraft=(await bad.json()) as {data:{draftId:string}};const validated=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${badDraft.data.draftId}/validate`,{});const validatedBody=await validated.json() as {data:{validationIssues:Array<{severity:string}>}};assert.ok(validatedBody.data.validationIssues.some((item)=>item.severity==='ERROR'));const badSave=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${badDraft.data.draftId}/save`,{});assert.equal(badSave.status,409);assert.equal(((await badSave.json()) as {error:{code:string}}).error.code,'RUNTIME_CONFIG_VALIDATION_FAILED')
  const a=await (await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'PARSER',endpointAlias:'parser-a'}]})).json() as {data:{draftId:string}};const b=await (await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'RENDERER',endpointAlias:'renderer-b'}]})).json() as {data:{draftId:string}};await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${a.data.draftId}/validate`,{});await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${b.data.draftId}/validate`,{});assert.equal((await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${a.data.draftId}/save`,{})).status,200);const conflict=await post(baseUrl,`/api/industry/v1/runtime/config/drafts/${b.data.draftId}/save`,{});assert.equal(conflict.status,409);assert.equal(((await conflict.json()) as {error:{code:string}}).error.code,'RUNTIME_CONFIG_VERSION_CONFLICT')
})})

test('runtime drafts are project isolated and operations approvals are display-only blocked checks',async()=>{await withServer(makeRouter(['runtime.read','runtime.config.edit','operations.read']),async(baseUrl)=>{
  await selectProject(baseUrl);const created=await (await post(baseUrl,'/api/industry/v1/runtime/config/drafts',{changes:[{component:'SANDBOX',endpointAlias:'sandbox-candidate'}]})).json() as {data:{draftId:string}};await selectProject(baseUrl,'project-2');const hidden=await fetch(`${baseUrl}/api/industry/v1/runtime/config/drafts/${encodeURIComponent(created.data.draftId)}`);assert.equal(hidden.status,404)
