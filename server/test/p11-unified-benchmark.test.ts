...
test('waiver is separate from PI gate, unique while active, and never makes a FAIL run baseline eligible',async()=>{await withServer(makeRouter(['eval.read','eval.waiver.create','eval.baseline.accept']),async(baseUrl)=>{
