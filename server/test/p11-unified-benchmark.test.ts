...
test('baseline requires explicit accept and rejects incomplete or waived FAIL runs',async()=>{await withServer(makeRouter(['eval.read','eval.baseline.accept','eval.waiver.create']),async(baseUrl)=>{
...
test('waiver is separate from PI gate and never makes a FAIL run baseline eligible',async()=>{await withServer(makeRouter(['eval.read','eval.waiver.create','eval.baseline.accept']),async(baseUrl)=>{
  ...
  const waiver=await post(baseUrl,'/api/industry/v1/eval/p11/runs/run-p11-broken-v0/waivers',{reason:'documented temporary release exception for external validation',expiresAt:new Date(Date.now()+86400000).toISOString()});assert.equal(waiver.status,201);const waiverBody=await waiver.json() as {data:{status:string;originalGate:string;releaseDisposition:string;createdBy:string}};assert.equal(waiverBody.data.status,'ACTIVE');assert.equal(waiverBody.data.originalGate,'FAIL');assert.equal(waiverBody.data.releaseDisposition,'WAIVED');assert.equal(waiverBody.data.createdBy,'user-p11')
  const decision=await (await fetch(`${baseUrl}/api/industry/v1/eval/p11/runs/run-p11-broken-v0/gate`)).json() as {data:{gate:{status:string;source:string};releaseDisposition:string;baselineEligible:boolean;baselineBlockReasons:string[]}};assert.equal(decision.data.gate.status,'FAIL');assert.equal(decision.data.gate.source,'PI');assert.equal(decision.data.releaseDisposition,'WAIVED');assert.equal(decision.data.baselineEligible,false);assert.ok(decision.data.baselineBlockReasons.length>0)
...
test('P11 has no Manual PASS Force PASS or generic promote endpoint and keeps runs project-isolated',async()=>{await withServer(makeRouter(['eval.read','eval.run','eval.baseline.accept','eval.waiver.create']),async(baseUrl)=>{
  ...
  await selectProject(baseUrl,'project-2');const hidden=await fetch(`${baseUrl}/api/industry/v1/eval/p11/runs/run-p11-candidate-v2`);assert.equal(hidden.status,200);const project2=await hidden.json() as {data:{projectId:string}};assert.equal(project2.data.projectId,'project-2')
  const foreign=await fetch(`${baseUrl}/api/industry/v1/eval/p11/runs/run-does-not-exist`);assert.equal(foreign.status,404)
})})
