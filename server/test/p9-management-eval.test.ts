...
test('Sandbox forbids arbitrary SQL/Python/queryId input and blocks dangerous capabilities before query execution',async()=>{await withServer(makeRouter(['sandbox.read','sandbox.run']),async(baseUrl)=>{
  ...
  for(const injected of [{generatedSql:'DELETE FROM engineering_position'},{pythonSource:'import os'},{queryId:'client-query'},{projectId:'project-2'}]){const response=await fetch(`${baseUrl}/api/industry/v1/sandbox/runs`,{method:'POST',headers:headers(),body:JSON.stringify({goal:'safe query',budget:sandboxBudget,...injected})});assert.equal(response.status,400)}
  const safe=await fetch(`${baseUrl}/api/industry/v1/sandbox/runs`,{method:'POST',headers:headers(),body:JSON.stringify({goal:'查询未完成工程部位',budget:sandboxBudget,scenario:'SAFE_READ'})});assert.equal(safe.status,201);const safeBody=await safe.json() as {data:{runId:string;projectId:string;generatedSql:string;queryId:string|null;runtimeAttestation:{verified:boolean;networkDisabled:boolean;processSpawnDisabled:boolean};output:{rowCount:number;bytes:number;rows:Array<{id:string}>};budget:{maxRows:number;maxBytes:number};validation:Array<{severity:string}>}};assert.equal(safeBody.data.projectId,'project-1');assert.match(safeBody.data.generatedSql,/^SELECT id, name, status/u);assert.ok(safeBody.data.queryId?.startsWith('query-'));assert.equal(safeBody.data.runtimeAttestation.verified,true);assert.equal(safeBody.data.runtimeAttestation.networkDisabled,true);assert.equal(safeBody.data.runtimeAttestation.processSpawnDisabled,true);assert.ok(safeBody.data.output.rowCount<=safeBody.data.budget.maxRows);assert.ok(safeBody.data.output.bytes<=safeBody.data.budget.maxBytes);assert.equal(typeof safeBody.data.output.rows[0]?.id,'string');assert.equal(safeBody.data.output.rows[0]?.id,'123456789012345678');assert.ok(safeBody.data.validation.every((item)=>item.severity!=='BLOCK'))
  for(const scenario of ['WRITE_SQL','SELECT_STAR','LOAD_FILE','SYSTEM_SCHEMA','PYTHON_IMPORT_OPEN_NETWORK_PROCESS'] as const){const response=await fetch(`${baseUrl}/api/industry/v1/sandbox/runs`,{method:'POST',headers:headers(),body:JSON.stringify({goal:`blocked ${scenario}`,budget:sandboxBudget,scenario})});assert.equal(response.status,201);const body=await response.json() as {data:{status:string;queryId:string|null;validation:Array<{severity:string}>}};assert.equal(body.data.status,'BLOCKED');assert.equal(body.data.queryId,null);assert.ok(body.data.validation.some((item)=>item.severity==='BLOCK'))}
  await selectProject(baseUrl,'project-2');const hidden=await fetch(`${baseUrl}/api/industry/v1/sandbox/runs/${encodeURIComponent(safeBody.data.runId)}`);assert.equal(hidden.status,404)
})})
...
test('P9 eval guarded profiles pass and broken profiles expose every specialist gate',async()=>{await withServer(makeRouter(['eval.read','eval.run']),async(baseUrl)=>{
  ...
  const brokenReport=await (await fetch(`${baseUrl}/api/industry/v1/eval/p9/runs/run-p9-report-broken-v0`)).json() as {data:{metrics:{releaseGate:string;hiddenFieldBlockedRate:number;evidenceCoverageRate:number;svgScriptBlockedRate:number;pathTraversalBlockedRate:number}}};assert.equal(brokenReport.data.metrics.releaseGate,'FAIL');assert.equal(brokenReport.data.metrics.hiddenFieldBlockedRate,0);assert.equal(brokenReport.data.metrics.evidenceCoverageRate,.5);assert.equal(brokenReport.data.metrics.svgScriptBlockedRate,0);assert.equal(brokenReport.data.metrics.pathTraversalBlockedRate,0)
  const brokenSandbox=await (await fetch(`${baseUrl}/api/industry/v1/eval/p9/runs/run-p9-sandbox-broken-v0`)).json() as {data:{metrics:{releaseGate:string;writeSqlRejectRate:number;selectStarRejectRate:number;loadFileRejectRate:number;pythonCapabilityBlockedRate:number;attestationVerifiedRate:number}}};assert.equal(brokenSandbox.data.metrics.releaseGate,'FAIL');assert.equal(brokenSandbox.data.metrics.writeSqlRejectRate,0);assert.equal(brokenSandbox.data.metrics.selectStarRejectRate,0);assert.equal(brokenSandbox.data.metrics.loadFileRejectRate,0);assert.equal(brokenSandbox.data.metrics.pythonCapabilityBlockedRate,0);assert.equal(brokenSandbox.data.metrics.attestationVerifiedRate,0)
