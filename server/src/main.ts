...
function parsePiCommand(raw: string | undefined): string[] {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.every((entry: unknown) => typeof entry === 'string')) return parsed
  } catch { /* invalid JSON falls through */ }
  console.error('[pi-web] invalid PI_CMD, falling back to default')
  return DEFAULT_PI_CMD
}
function parseMode(raw: string | undefined): PiWebMode {
  if (raw === undefined || raw === 'local') return 'local'
  if (raw === 'control-plane') return 'control-plane'
  throw new Error('PI_WEB_MODE must be "local" or "control-plane"')
}
...
const server = http.createServer((request, response) => {
  void (async () => { if (await industryRouter(request, response)) return; await staticHandler(request, response) })().catch(() => {
    if (!response.headersSent) response.writeHead(500)
    response.end('internal server error')
  })
})
attachBridge({ server, piCommand: PI_CMD, piCwd: PI_CWD, mode: MODE })
