...
export async function handleRuntimeRoute(options: RuntimeRouteOptions): Promise<boolean> {
  ...
  if(path==='/api/industry/v1/runtime/config/drafts'&&options.request.method==='POST'){requireConfigEdit(options.principal);const body=await readJsonBody(options.request,options.bodyLimitBytes);return sendData(options.response,await options.client.createRuntimeConfigDraft(options.context,parseCreateDraft(body)),201)}
  ...
  if(actionMatch!==null&&options.request.method==='POST'){
    requireConfigEdit(options.principal);const body=await readJsonBody(options.request,options.bodyLimitBytes);requireEmptyObject(body);const draftId=decode(actionMatch[1]);const action=actionMatch[2]
...
function requireConfigEdit(principal:AuthPrincipal):void{if(principal.permissions.includes('runtime.admin'))return;if(principal.permissions.includes('runtime.read')&&principal.permissions.includes('runtime.config.edit'))return;throw new IndustryAgentClientError('RUNTIME_ACCESS_DENIED','runtime config changes require runtime.read + runtime.config.edit',403)}
