...
MockIndustryAgentClient.prototype.listReports = async function listReports(context) { const projectId=requireProject(context);const store=reportStore(this,projectId);seedReports(store,projectId);return structuredClone([...store.values()].sort((a,b)=>b.createdAt.localeCompare(a.createdAt))) }
MockIndustryAgentClient.prototype.getReport = async function getReport(context,reportId) { const projectId=requireProject(context);const store=reportStore(this,projectId);seedReports(store,projectId);const report=store.get(reportId);if(report===undefined)throw new IndustryAgentClientError('REPORT_NOT_FOUND','report not found',404);return structuredClone(report) }
MockIndustryAgentClient.prototype.createReportDownloadGrant = async function createReportDownloadGrant(context,reportId) { const report=await this.getReport(context,reportId);if(report.status!=='READY')throw new IndustryAgentClientError('REPORT_DOWNLOAD_BLOCKED','report is not eligible for download',409);return{downloadId:`report-download-${randomUUID()}`,reportId:report.reportId,fileName:report.fileName,contentDisposition:'attachment',authorized:true,expiresAt:new Date(Date.now()+5*60_000).toISOString(),artifactRef:`mock-report-artifact:${report.reportId}:${randomUUID()}`} }
...
function buildSandboxRun(runId:string,projectId:string,goal:string,budget:SandboxBudget,scenario:SandboxScenario):SandboxRun{
  const createdAt='2026-09-13T08:35:00.000Z';const nominalRuntimeMs=250
  ...
  let generatedSql="SELECT id, name, status FROM engineering_position WHERE project_id = :trusted_project_id AND status <> 'DONE' LIMIT 100";let pythonSource='rows = query_result\nsummary = {"count": len(rows)}';let terminationReason:SandboxRun['terminationReason']='SUCCESS';const validation:SandboxRun['validation'][number][]=[];let attestationVerified=true
  ...
  if(!blockedRun&&budget.timeoutMs<nominalRuntimeMs){validation.push({code:'TIMEOUT_ENFORCED',severity:'INFO',message:'sandbox stopped at configured timeout'});terminationReason='TIMEOUT'}
  ...
  if(!blockedRun&&terminationReason!=='TIMEOUT'&&(rowLimited.length<candidates.length||rows.length<rowLimited.length))terminationReason='PAYLOAD_BUDGET'
