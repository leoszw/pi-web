...
MockIndustryAgentClient.prototype.listReports = async function listReports(context) {
  ...
  const store = reportStore(this, projectId)
  seedReports(store, projectId)
...
MockIndustryAgentClient.prototype.createReportDownloadGrant = async function createReportDownloadGrant(context, reportId) {
  ...
  const grant: ReportDownloadGrant = {
    downloadId: `report-download-${randomUUID()}`,
    reportId: report.reportId,
    fileName: report.fileName,
    contentDisposition: 'attachment',
    authorized: true,
    expiresAt,
    artifactRef: `mock-report-artifact:${report.reportId}:${randomUUID()}`,
  }
  return grant
}

MockIndustryAgentClient.prototype.listSandboxRuns = async function listSandboxRuns(context) {
  const projectId = requireProject(context)
  const store = sandboxStore(this, projectId)
  seedSandbox(store, projectId)
  return structuredClone([...store.values()].sort((a, b) => b.createdAt.localeCompare(a.createdAt)))
}

MockIndustryAgentClient.prototype.startSandboxRun = async function startSandboxRun(context, request) {
  const projectId = requireProject(context)
  validateSandboxRequest(request)
  const runId = `sandbox-${randomUUID()}`
  const run = buildSandboxRun(runId, projectId, request.goal, request.budget, request.scenario ?? 'SAFE_READ')
  sandboxStore(this, projectId).set(run.runId, run)
  return structuredClone(run)
}

MockIndustryAgentClient.prototype.getSandboxRun = async function getSandboxRun(context, runId) {
  const projectId = requireProject(context)
  const store = sandboxStore(this, projectId)
  seedSandbox(store, projectId)
  const run = store.get(runId)
  if (run === undefined) throw new IndustryAgentClientError('SANDBOX_RUN_NOT_FOUND', 'sandbox run not found', 404)
  return structuredClone(run)
}

function reportStore(client: MockIndustryAgentClient, projectId: string): Map<string, ReportArtifact> {
  let projects = REPORTS.get(client)
  if (projects === undefined) { projects = new Map(); REPORTS.set(client, projects) }
  let store = projects.get(projectId)
  if (store === undefined) { store = new Map(); projects.set(projectId, store) }
  return store
}

function sandboxStore(client: MockIndustryAgentClient, projectId: string): Map<string, SandboxRun> {
  let projects = SANDBOX.get(client)
  if (projects === undefined) { projects = new Map(); SANDBOX.set(client, projects) }
  let store = projects.get(projectId)
  if (store === undefined) { store = new Map(); projects.set(projectId, store) }
  return store
}
...
function seedReports(store: Map<string, ReportArtifact>, projectId: string): void {
  ...
  const pdf: ReportArtifact = {
    reportId: 'report-progress-v1', projectId, title: '工程进度周报', format: 'PDF', fileName: 'project-progress-weekly.pdf', sizeBytes: 184_320, status: 'READY', mimeType: 'application/pdf',
    preview: '本周完成 12 个工程部位，未完成 3 个。所有结论均可追溯到查询结果与知识证据。',
    metadata: { reportVersion: '1.0.0', renderer: 'mock-safe-pdf-v1', locale: 'zh-CN' },
    evidence: [
      { evidenceId: 'evidence-query-1', label: '未完成工程部位查询', sourceType: 'QUERY_RESULT', sourceRef: 'query:p9-progress-1', covered: true },
      { evidenceId: 'evidence-rag-1', label: '施工规范引用', sourceType: 'RAG_CHUNK', sourceRef: 'chunk:spec-v3-42', covered: true },
    ],
    lineage: [
      { nodeId: 'source-query', type: 'SOURCE', label: 'query:p9-progress-1', parentIds: [] },
      { nodeId: 'source-rag', type: 'SOURCE', label: 'chunk:spec-v3-42', parentIds: [] },
      { nodeId: 'transform-summary', type: 'TRANSFORM', label: 'weekly-progress-summary', parentIds: ['source-query','source-rag'] },
      { nodeId: 'report', type: 'REPORT', label: 'project-progress-weekly.pdf', parentIds: ['transform-summary'] },
    ], security: baseSecurity, traceId: `trace-report-${projectId}-progress`, createdAt: '2026-09-13T08:30:00.000Z',
  }
  const xlsx: ReportArtifact = {
    ...pdf, reportId: 'report-boq-v1', title: '工程量清单快照', format: 'XLSX', fileName: 'boq-snapshot.xlsx', mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', sizeBytes: 98_304,
    preview: '只包含授权列：清单编码、名称、规格、数量、单位。隐藏字段未导出。', metadata: { reportVersion: '1.0.0', renderer: 'mock-safe-xlsx-v1', locale: 'zh-CN' },
    evidence: [{ evidenceId: 'evidence-query-2', label: '清单查询快照', sourceType: 'QUERY_RESULT', sourceRef: 'query:p9-boq-1', covered: true }],
    lineage: [{ nodeId: 'source-query', type: 'SOURCE', label: 'query:p9-boq-1', parentIds: [] }, { nodeId: 'report', type: 'REPORT', label: 'boq-snapshot.xlsx', parentIds: ['source-query'] }], traceId: `trace-report-${projectId}-boq`, createdAt: '2026-09-13T08:31:00.000Z',
  }
  store.set(pdf.reportId, pdf); store.set(xlsx.reportId, xlsx)
}

function seedSandbox(store: Map<string, SandboxRun>, projectId: string): void {
  if (store.size > 0) return
  const safe = buildSandboxRun('sandbox-safe-read-v1', projectId, '统计当前项目未完成工程部位', DEFAULT_BUDGET, 'SAFE_READ')
  const blocked = buildSandboxRun('sandbox-write-blocked-v1', projectId, '尝试修改工程部位状态', DEFAULT_BUDGET, 'WRITE_SQL')
...
function buildSandboxRun(runId: string, projectId: string, goal: string, budget: SandboxBudget, scenario: SandboxScenario): SandboxRun {
  ...
  const rowBudget = Math.max(0, Math.min(budget.maxRows, scenario === 'PAYLOAD_BUDGET' ? budget.maxRows : 2))
  const allRows = [
    { id:'123456789012345678', name:'一号墩钢筋工程', status:'IN_PROGRESS' },
    { id:'223456789012345678', name:'二号墩基础', status:'PENDING' },
  ]
  const rows = blockedRun ? [] : allRows.slice(0, rowBudget)
  const rawBytes = Buffer.byteLength(JSON.stringify(rows), 'utf8')
  const bytes = Math.min(rawBytes, budget.maxBytes)
  if (!blockedRun && bytes < rawBytes) terminationReason = 'PAYLOAD_BUDGET'
  ...
  return {
    runId, projectId, goal, status, scenario, budget:{...budget}, schema, generatedSql, validation, queryId,
    brokerPolicies:[{policy:'sql_mode',value:'read_only_select_cte'},{policy:'project_scope',value:'trusted_context_only'},{policy:'select_star',value:'deny'},{policy:'system_schema',value:'deny'},{policy:'network',value:'disabled'},{policy:'process_spawn',value:'disabled'}],
    pythonSource, pythonHash:createHash('sha256').update(pythonSource).digest('hex'), runtimeAttestation:attestation,
    output:{columns:['id','name','status'],rows,rowCount:rows.length,bytes},
    lineage:[{nodeId:'goal',type:'GOAL',label:goal,parentIds:[]},{nodeId:'sql',type:'SQL',label:'generated SQL',parentIds:['goal']},{nodeId:'query',type:'QUERY',label:queryId ?? 'blocked before query',parentIds:['sql']},{nodeId:'python',type:'PYTHON',label:'post-process',parentIds:['query']},{nodeId:'output',type:'OUTPUT',label:'sandbox output',parentIds:['python']}],
    terminationReason, traceId:`trace-${runId}`, createdAt, completedAt:new Date(Date.parse(createdAt)+Math.min(budget.timeoutMs,250)).toISOString(),
  }
}

function validateSandboxRequest(request: StartSandboxRunRequest): void {
  if (request.goal.trim() === '' || request.goal.length > 2000) throw new IndustryAgentClientError('SANDBOX_GOAL_INVALID','sandbox goal is required and must be at most 2000 characters',400)
  const { maxRows, maxBytes, timeoutMs } = request.budget
  if (!Number.isInteger(maxRows) || maxRows < 1 || maxRows > 10_000) throw new IndustryAgentClientError('SANDBOX_BUDGET_INVALID','maxRows must be between 1 and 10000',400)
  if (!Number.isInteger(maxBytes) || maxBytes < 128 || maxBytes > 10_000_000) throw new IndustryAgentClientError('SANDBOX_BUDGET_INVALID','maxBytes is invalid',400)
  if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000) throw new IndustryAgentClientError('SANDBOX_BUDGET_INVALID','timeoutMs is invalid',400)
}
