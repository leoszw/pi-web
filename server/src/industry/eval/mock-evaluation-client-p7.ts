...
interface StoredP7Run {
  summary: P7EvalRunSummary
...
const DATASET_BY_DOMAIN: Readonly<Record<P7EvalDomain, string>> = {
  NORMALIZATION: 'normalization-safety-v1',
  ENTITY: 'entity-safety-v1',
  TOOL: 'tool-safety-v1',
  MEMORY: 'memory-safety-v1',
}
...
MockEvaluationClient.prototype.listP7EvalCases = async function listP7EvalCases(context, domain) {
  requireProject(context)
  return structuredClone(casesFor(domain))
}

MockEvaluationClient.prototype.listP7EvalRuns = async function listP7EvalRuns(context, domain) {
  const store = runStore(this, requireProject(context))
  return [...store.values()].filter((item) => item.summary.domain === domain).map((item) => structuredClone(item.summary)).sort((a, b) => b.startedAt.localeCompare(a.startedAt))
}

MockEvaluationClient.prototype.startP7EvalRun = async function startP7EvalRun(context, domain, request) {
  ...
  const run = buildRun(`run-p7-${domain.toLowerCase()}-${randomUUID()}`, domain, request.variantId, projectId)
  runStore(this, projectId).set(run.summary.runId, run)
  return structuredClone(run.summary)
}

MockEvaluationClient.prototype.getP7EvalRun = async function getP7EvalRun(context, runId) {
  return structuredClone(runById(runStore(this, requireProject(context)), runId).summary)
}

MockEvaluationClient.prototype.listP7EvalObservations = async function listP7EvalObservations(context, runId) {
  return structuredClone(runById(runStore(this, requireProject(context)), runId).observations)
}

MockEvaluationClient.prototype.listP7EvalFailures = async function listP7EvalFailures(context, runId) {
  const observations = runById(runStore(this, requireProject(context)), runId).observations
  return structuredClone(observations.filter((item) => !item.passed).map((item) => ({
    caseId: item.caseId,
    domain: item.domain,
    title: item.title,
    ...(item.failureStage === undefined ? {} : { failureStage: item.failureStage }),
    reasons: item.reasons,
    traceId: item.traceId,
  })))
}

MockEvaluationClient.prototype.listP7Drafts = async function listP7Drafts(context) {
  const projectId = requireProject(context)
  return structuredClone(draftStore(this, projectId))
}

MockEvaluationClient.prototype.createP7DraftFromTrace = async function createP7DraftFromTrace(context, traceId, targetDomain, sourceTraceName) {
  const projectId = requireProject(context)
  const draft: P7EvalDraft = {
    draftId: `draft-p7-${randomUUID()}`,
    projectId,
    ...
    status: 'DRAFT',
    reviewed: false,
    query: sourceTraceName,
    createdAt: new Date().toISOString(),
  }
  draftStore(this, projectId).unshift(draft)
  return structuredClone(draft)
}

function runStore(client: MockEvaluationClient, projectId: string): Map<string, StoredP7Run> {
  let projects = RUNS.get(client)
  if (projects === undefined) { projects = new Map(); RUNS.set(client, projects) }
  let store = projects.get(projectId)
  if (store === undefined) {
    store = new Map()
    for (const domain of domains()) {
      for (const variant of ['p7-broken-v0', 'p7-guarded-v1'] as const) {
        const run = buildRun(`run-p7-${domain.toLowerCase()}-${variant === 'p7-broken-v0' ? 'broken-v0' : 'guarded-v1'}`, domain, variant, projectId)
...
function draftStore(client: MockEvaluationClient, projectId: string): P7EvalDraft[] {
  let projects = DRAFTS.get(client)
  if (projects === undefined) { projects = new Map(); DRAFTS.set(client, projects) }
  let store = projects.get(projectId)
  if (store === undefined) { store = []; projects.set(projectId, store) }
  return store
}

function buildRun(runId: string, domain: P7EvalDomain, variantId: P7EvalVariant, projectId: string): StoredP7Run {
  const observations = casesFor(domain).map((testCase) => observationFor(runId, variantId, projectId, testCase))
  const now = new Date().toISOString()
  return {
    summary: {
      runId,
      domain,
      datasetId: DATASET_BY_DOMAIN[domain],
      ...
      startedAt: now,
      completedAt: now,
      metrics: metricsFor(domain, observations),
    },
    observations,
  }
}
...
function observationFor(runId: string, variantId: P7EvalVariant, projectId: string, testCase: P7EvalCase): P7EvalObservation {
  ...
  if (testCase.domain === 'NORMALIZATION') {
    const broken = new Set(['normalization-002', 'normalization-004', 'normalization-008', 'normalization-009']).has(testCase.caseId)
    return broken
  ...
  if (testCase.domain === 'ENTITY') {
    const stage = entityStage(testCase.caseId)
    return failObservation(runId, projectId, testCase, stage, testCase.expectedEntityId, 'unresolved', [`entity resolution failed at ${stage}`], { mention: testCase.mention, expectedScope: testCase.expectedScope })
  }
...
function passObservation(runId: string, projectId: string, testCase: P7EvalCase): P7EvalObservation {
  ...
  return {
    schemaVersion: 'eval-observation-v1', observationId: `${runId}:${testCase.caseId}`, runId, caseId: testCase.caseId,
    domain: testCase.domain, title: testCase.title, passed: true, expectedSummary: expected, actualSummary: expected,
    reasons: [], details: passDetails(testCase), traceId: `trace-p7-${safeId(projectId)}-${testCase.caseId}`,
  }
}
...
function failObservation(runId: string, projectId: string, testCase: P7EvalCase, failureStage: NonNullable<P7EvalObservation['failureStage']>, expectedSummary: string, actualSummary: string, reasons: readonly string[], details: Readonly<Record<string, unknown>>): P7EvalObservation {
  return {
    schemaVersion: 'eval-observation-v1', observationId: `${runId}:${testCase.caseId}`, runId, caseId: testCase.caseId,
    domain: testCase.domain, title: testCase.title, passed: false, expectedSummary, actualSummary, failureStage, reasons, details,
    traceId: `trace-p7-${safeId(projectId)}-${testCase.caseId}`,
  }
}
...
function metricsFor(domain: P7EvalDomain, observations: readonly P7EvalObservation[]): P7EvalMetrics {
  ...
  const base: P7EvalMetrics = {
    sampleCount: observations.length,
    passedCount,
    passRate: passedCount / observations.length,
    releaseGate: passedCount === observations.length ? 'PASS' : 'FAIL',
    releaseGateReasons: [...new Set(observations.flatMap((item) => item.reasons))],
  }
  ...
  if (domain === 'TOOL') {
    ...
    return {
      ...base,
      toolSelectionAccuracy: observations.filter((item) => detailBool(item, 'selectionExact', true)).length / n,
      toolArgumentExactRate: observations.filter((item) => detailBool(item, 'argumentExact', true)).length / n,
      toolMissingRequiredRate: observations.filter((item) => detailBool(item, 'missingRequired', false)).length / n,
      toolUnknownArgumentRate: observations.filter((item) => detailBool(item, 'unknownArgument', false)).length / n,
      toolScopeInjectionBlockedRate: observations.filter((item) => detailBool(item, 'scopeInjectionBlocked', true)).length / n,
      toolUnnecessaryToolRate: observations.filter((item) => detailBool(item, 'unnecessaryTool', false)).length / n,
      toolSequenceAccuracy: observations.filter((item) => detailBool(item, 'sequenceExact', true)).length / n,
    }
  }
  const n = observations.length
  return {
    ...base,
    memoryResolutionAccuracy: observations.filter((item) => detailBool(item, 'resolutionExact', true)).length / n,
    memoryProjectIsolationRate: observations.filter((item) => detailBool(item, 'projectIsolation', true)).length / n,
    memoryTtlPolicyRate: observations.filter((item) => detailBool(item, 'ttlPolicy', true)).length / n,
    memorySourcePolicyRate: observations.filter((item) => detailBool(item, 'sourcePolicy', true)).length / n,
  }
}

function passDetails(testCase: P7EvalCase): Readonly<Record<string, unknown>> {
  if (testCase.domain === 'TOOL') return { selectionExact: true, argumentExact: true, missingRequired: false, unknownArgument: false, scopeInjectionBlocked: true, unnecessaryTool: false, sequenceExact: true }
  if (testCase.domain === 'MEMORY') return { resolutionExact: true, projectIsolation: true, ttlPolicy: true, sourcePolicy: true }
  return {}
}

function expectedSummary(testCase: P7EvalCase): string {
  if (testCase.domain === 'NORMALIZATION') return testCase.expectedNormalized
  if (testCase.domain === 'ENTITY') return testCase.expectedEntityId
  if (testCase.domain === 'TOOL') return testCase.expectedTools.length === 0 ? 'no tool' : testCase.expectedTools.join(' → ')
  return testCase.expectedResolution
}

function casesFor(domain: P7EvalDomain): readonly P7EvalCase[] {
  if (domain === 'NORMALIZATION') return NORMALIZATION_CASES
  if (domain === 'ENTITY') return ENTITY_CASES
  if (domain === 'TOOL') return TOOL_CASES
  return MEMORY_CASES
}

function norm(caseId: string, category: NormalizationEvalCase['category'], input: string, expectedNormalized: string): NormalizationEvalCase {
  return { schemaVersion: 'eval-case-v1', caseId, datasetId: DATASET_BY_DOMAIN.NORMALIZATION, datasetVersion: DATASET_VERSION, domain: 'NORMALIZATION', title: `${category} normalization`, tags: ['normalization', category.toLowerCase()], critical: true, category, input, expectedNormalized }
}
function entity(caseId: string, mention: string, expectedEntityId: string, expectedScope: string, tag: string): EntityEvalCase {
  return { schemaVersion: 'eval-case-v1', caseId, datasetId: DATASET_BY_DOMAIN.ENTITY, datasetVersion: DATASET_VERSION, domain: 'ENTITY', title: `Entity ${tag}`, tags: ['entity', tag], critical: true, mention, expectedEntityId, expectedScope }
}
function tool(caseId: string, query: string, expectedTools: readonly string[], expectedArguments: Readonly<Record<string, unknown>>, requiredArguments: readonly string[], forbiddenArguments: readonly string[], expectedSequence: readonly string[], tag: string): ToolEvalCase {
  return { schemaVersion: 'eval-case-v1', caseId, datasetId: DATASET_BY_DOMAIN.TOOL, datasetVersion: DATASET_VERSION, domain: 'TOOL', title: `Tool ${tag}`, tags: ['tool', tag], critical: true, query, expectedTools, expectedArguments, requiredArguments, forbiddenArguments, expectedSequence }
}
function memory(caseId: string, utterance: string, previousContext: readonly string[], expectedResolution: string, sourcePolicy: MemoryEvalCase['sourcePolicy'], ttlValid: boolean, tag: string): MemoryEvalCase {
  return { schemaVersion: 'eval-case-v1', caseId, datasetId: DATASET_BY_DOMAIN.MEMORY, datasetVersion: DATASET_VERSION, domain: 'MEMORY', title: `Memory ${tag}`, tags: ['memory', tag], critical: true, utterance, previousContext, expectedResolution, expectedProjectId: 'CURRENT_PROJECT', ttlValid, sourcePolicy }
}

function entityStage(caseId: string): EntityFailureStage {
  if (caseId === 'entity-001') return 'MENTION'
  if (caseId === 'entity-002') return 'CANDIDATE_GENERATION'
  if (caseId === 'entity-003') return 'RANKING'
  if (caseId === 'entity-004') return 'SCOPE'
  return 'AMBIGUITY'
}
function isEntityStage(value: P7EvalObservation['failureStage']): value is EntityFailureStage {
  return value === 'MENTION' || value === 'CANDIDATE_GENERATION' || value === 'RANKING' || value === 'SCOPE' || value === 'AMBIGUITY'
}
function detailBool(item: P7EvalObservation, key: string, fallback: boolean): boolean {
  const value = item.details[key]
  return typeof value === 'boolean' ? value : fallback
}
function runById(store: Map<string, StoredP7Run>, runId: string): StoredP7Run {
  const run = store.get(runId)
  if (run === undefined) throw new EvaluationClientError('EVAL_RUN_NOT_FOUND', 'P7 evaluation run not found', 404)
  return run
}
function requireVariant(value: string): asserts value is P7EvalVariant {
  if (value !== 'p7-broken-v0' && value !== 'p7-guarded-v1') throw new EvaluationClientError('EVAL_VARIANT_NOT_FOUND', 'P7 evaluation variant not found', 404)
}
function requireProject(context: TrustedRequestContext): string {
  if (context.projectId === null) throw new EvaluationClientError('EVAL_PROJECT_REQUIRED', 'select an active project before using P7 evaluation', 409)
