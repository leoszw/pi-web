...
function parseLabel(value:unknown):LabelOnlineFeedbackRequest{const row=record(value);only(row,['label','tags','difficulty','notes']);if(!Array.isArray(row.tags)||!row.tags.every((item)=>typeof item==='string'))throw new RequestBodyError('INVALID_JSON','tags must be an array of strings',400);return{label:str(row.label,'label'),tags:row.tags,difficulty:difficulty(row.difficulty),...(row.notes===undefined?{}:{notes:str(row.notes,'notes')})}}
function parseReview(value:unknown):ReviewOnlineFeedbackRequest{const row=record(value);only(row,['approved','reviewNote']);if(row.approved!==true)throw new RequestBodyError('INVALID_JSON','approved must be true for review',400);return{approved:true,...(row.reviewNote===undefined?{}:{reviewNote:str(row.reviewNote,'reviewNote')})}}
...
function requireRead(principal:AuthPrincipal):void{if(admin(principal)||principal.permissions.includes('quality.read'))return;throw new QualityAccessError('QUALITY_ACCESS_DENIED','missing permission: quality.read',403)}
function requireEdit(principal:AuthPrincipal):void{if(admin(principal)||(principal.permissions.includes('quality.read')&&principal.permissions.includes('quality.feedback.edit')))return;throw new QualityAccessError('QUALITY_ACCESS_DENIED','quality.read + quality.feedback.edit are required',403)}
function requireReview(principal:AuthPrincipal):void{if(admin(principal)||(principal.permissions.includes('quality.read')&&principal.permissions.includes('quality.feedback.review')))return;throw new QualityAccessError('QUALITY_ACCESS_DENIED','quality.read + quality.feedback.review are required',403)}
function requireVersion(principal:AuthPrincipal):void{if(admin(principal)||(principal.permissions.includes('quality.read')&&principal.permissions.includes('quality.feedback.review')&&principal.permissions.includes('eval.dataset.edit')))return;throw new QualityAccessError('QUALITY_ACCESS_DENIED','quality.read + quality.feedback.review + eval.dataset.edit are required',403)}
function requireTraceRead(principal:AuthPrincipal):void{if(admin(principal)||principal.permissions.includes('trace.admin')||principal.permissions.includes('trace.read.basic'))return;throw new QualityAccessError('TRACE_ACCESS_DENIED','missing permission: trace.read.basic',403)}
function admin(principal:AuthPrincipal):boolean{return principal.permissions.includes('quality.admin')||principal.permissions.includes('eval.admin')}
