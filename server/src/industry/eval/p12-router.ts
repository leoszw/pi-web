...
function requireVersion(principal:AuthPrincipal):void{if(isQualityAdmin(principal))return;const qualityAllowed=principal.permissions.includes('quality.read')&&principal.permissions.includes('quality.feedback.review');const datasetAllowed=principal.permissions.includes('eval.dataset.edit')||principal.permissions.includes('eval.admin');if(qualityAllowed&&datasetAllowed)return;throw new QualityAccessError('QUALITY_ACCESS_DENIED','quality.read + quality.feedback.review + eval.dataset.edit are required',403)}
