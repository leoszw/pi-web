...
MockIndustryAgentClient.prototype.listMultimodalAnalyses = async function listMultimodalAnalyses(context) {
  const store = analysisStore(this, requireProject(context))
  seedAnalyses(store, requireProject(context))
...
MockIndustryAgentClient.prototype.reviewMultimodalObservation = async function reviewMultimodalObservation(context, analysisId, observationId, request) {
  ...
  if (request.selectedEntityId !== undefined && !candidateIds.has(request.selectedEntityId)) {
    throw new IndustryAgentClientError('MULTIMODAL_ENTITY_NOT_ALLOWED', 'selected entity is outside the server-issued candidates', 400)
  }
  const allowedFields = new Set(observation.fields.map((field) => field.name))
  for (const key of Object.keys(request.correctedFields ?? {})) {
    if (!allowedFields.has(key)) throw new IndustryAgentClientError('MULTIMODAL_FIELD_NOT_ALLOWED', 'corrected field is not present in the observation schema', 400)
  }
  ...
  const next: MultimodalAnalysis = {
    ...analysis,
    observations: analysis.observations.map((item) => item.observationId === observationId ? updated : item),
...
function analysisStore(client: MockIndustryAgentClient, projectId: string): Map<string, MultimodalAnalysis> {
  let projects = ANALYSES.get(client)
  if (projects === undefined) { projects = new Map(); ANALYSES.set(client, projects) }
...
function loopStore(client: MockIndustryAgentClient, projectId: string): Map<string, AgentLoopRun> {
  let projects = LOOPS.get(client)
  if (projects === undefined) { projects = new Map(); LOOPS.set(client, projects) }
  let store = projects.get(projectId)
  if (store === undefined) { store = new Map(); projects.set(projectId, store) }
  return store
}
...
function buildAnalysis(analysisId: string, projectId: string, request: CreateMultimodalAnalysisRequest, now: string): MultimodalAnalysis {
  ...
  if (lower.includes('no-evidence')) {
    return { analysisId, projectId, image: { ...request }, observations: [], noEvidence: true, promptInjectionBlocked: false, createdAt: now, updatedAt: now }
  }
  ...
  const observation: MultimodalObservation = {
    observationId: `observation-${analysisId}-1`,
    label: 'bridge_pier_rebar',
    confidence: low ? 0.54 : 0.94,
    bbox: { x: 0.18, y: 0.14, width: 0.52, height: 0.68 },
    fields: [
      { name: 'diameter_mm', value: low ? '' : '25', confidence: low ? 0.3 : 0.92, missing: low },
      { name: 'concrete_grade', value: 'C30', confidence: low ? 0.61 : 0.9, missing: false },
    ],
    entityCandidates: [
      { entityId: 'engineering-position-123456789012345678', entityType: 'ENGINEERING_POSITION', name: '一号墩钢筋工程', confidence: low ? 0.58 : 0.93 },
      { entityId: 'engineering-position-223456789012345678', entityType: 'ENGINEERING_POSITION', name: '二号墩钢筋工程', confidence: 0.33 },
    ],
    missingFields: low ? ['diameter_mm'] : [],
    lowConfidence: low,
...
function requireAnalysis(client: MockIndustryAgentClient, projectId: string, analysisId: string): MultimodalAnalysis {
  const store = analysisStore(client, projectId)
...
function buildLoopRun(runId: string, projectId: string, goal: string, budget: AgentLoopBudget, scenario: NonNullable<StartAgentLoopRunRequest['scenario']>): AgentLoopRun {
  ...
  const step = (sequenceNo: number, phase: AgentLoopStep['phase'], title: string, detail: string, overrides: Partial<AgentLoopStep> = {}): AgentLoopStep => ({
    sequenceNo, phase, status: 'OK', title, detail, tokenUsage: 80, costUsd: 0.01,
    startedAt: new Date(Date.parse(started) + sequenceNo * 100).toISOString(),
    completedAt: new Date(Date.parse(started) + sequenceNo * 100 + 75).toISOString(),
    ...overrides,
  })
  ...
  if (scenario === 'REPLAN') {
    ...
    terminationReason = 'MAX_STEPS'; steps = [step(1,'PLAN','Plan','bounded loop'), step(2,'ACT','Act','query',{toolName:'query'}), step(3,'TERMINATE','Terminate','max steps reached')]
  } else if (scenario === 'MAX_TOOLS') {
    terminationReason = 'MAX_TOOLS'; steps = [step(1,'PLAN','Plan','bounded tools'), step(2,'ACT','Act','tool budget reached',{toolName:'query'}), step(3,'TERMINATE','Terminate','max tools reached')]
  } else if (scenario === 'TOKEN_COST') {
    terminationReason = budget.maxTokens <= 100 ? 'TOKEN_BUDGET' : 'COST_BUDGET'; steps = [step(1,'PLAN','Plan','budgeted execution'), step(2,'TERMINATE','Terminate','token/cost budget reached')]
  ...
  const totalTokens = scenario === 'TOKEN_COST' ? Math.max(budget.maxTokens, 100) : steps.reduce((sum, item) => sum + item.tokenUsage, 0)
  const costUsd = scenario === 'TOKEN_COST' && terminationReason === 'COST_BUDGET' ? budget.maxCostUsd : Number(steps.reduce((sum, item) => sum + item.costUsd, 0).toFixed(4))
  const elapsedMs = scenario === 'TIMEOUT' ? budget.timeoutMs : Math.max(steps.length * 100, 1)
  const usage = { steps: steps.length, toolCalls, inputTokens: Math.floor(totalTokens * 0.7), outputTokens: totalTokens - Math.floor(totalTokens * 0.7), totalTokens, costUsd, elapsedMs, complete: scenario !== 'USAGE_INCOMPLETE' }
