...
export function AppShell(props:AppShellProps){
  const{initialPath}=props;const[route,setRoute]=useState<AppRoute>(()=>resolveAppRoute(initialPath??currentPath()));const[projectScopeVersion,setProjectScopeVersion]=useState(0);const pathname=initialPath??currentPath();const retrievalRunId=retrievalRunIdFromPath(pathname);const retrievalDebugTraceId=retrievalDebugTraceIdFromPath(pathname);const mutationEvalRunId=mutationEvalRunIdFromPath(pathname);const traceEvalRunId=traceEvalRunIdFromPath(pathname);const ragEvalRunId=ragEvalRunIdFromPath(pathname);const mutationOperationId=mutationOperationIdFromPath(pathname);const traceId=traceIdFromPath(pathname);const contentScopeKey=route==='chat'?'chat':`industry-${projectScopeVersion}`
  ...
  </nav><ProjectSelector onProjectChanged={()=>setProjectScopeVersion((value)=>value+1)}/><div className="app-shell__content"><AppContent key={contentScopeKey} {...props} route={route} traceId={traceId} retrievalDebugTraceId={retrievalDebugTraceId} retrievalRunId={retrievalRunId} mutationEvalRunId={mutationEvalRunId} traceEvalRunId={traceEvalRunId} ragEvalRunId={ragEvalRunId} mutationOperationId={mutationOperationId}/></div></div>
}
