...
export function TraceEvalPage({
  ...
  if (snapshot === undefined) return <main className="eval-page"><h1>Trace / Token Eval</h1>{error === null ? <p>Loading trace evaluation…</p> : <p role="alert">{error}</p>}</main>
...
export function TraceEvalView({
  ...
  return <main className="eval-page" aria-labelledby="trace-eval-title">
    ...
    <div className="eval-heading-row">
      <div><h1 id="trace-eval-title">Trace / Token Eval</h1><p>Deterministic validation for trace completeness, sequence, token accounting, tool audit, redaction, and retrieval query latency.</p></div>
      <div className="eval-heading-actions"><a className="eval-primary-link" href="/industry/traces">Trace Explorer</a><a href="/industry/eval">Evaluation</a></div>
    </div>
    {error === null ? null : <p role="alert" className="eval-error">{error}</p>}

    <section className="eval-panel">
      <div className="eval-heading-row"><div><h2>Batch Run</h2><p>Dataset <code>{DATASET_ID}</code> · 6 critical observability gates.</p></div><div className="eval-heading-actions">
        <select aria-label="Trace eval variant" value={variantId} disabled={busy} onChange={(event) => onVariantChange(event.target.value as TraceEvalVariant)}><option value="trace-broken-v0">trace-broken-v0</option><option value="trace-guarded-v1">trace-guarded-v1</option></select>
        <button type="button" disabled={busy} onClick={onStartRun}>Run Trace Eval</button>
      </div></div>
    </section>

    {run === undefined || metrics === undefined ? <section className="eval-panel"><p>Select or start a run to inspect trace metrics.</p></section> : <>
      <section className="eval-summary-grid" aria-label="Trace evaluation summary">
        <Metric label="Release gate" value={metrics.releaseGate} critical={metrics.releaseGate === 'FAIL'} />
        <Metric label="Pass rate" value={percent(metrics.passRate)} critical={metrics.passRate < 1} />
        <Metric label="Trace completeness" value={percent(metrics.traceCompletenessRate)} critical={metrics.traceCompletenessRate < 1} />
        <Metric label="Sequence monotonic" value={percent(metrics.sequenceMonotonicRate)} critical={metrics.sequenceMonotonicRate < 1} />
        <Metric label="Token accounting" value={percent(metrics.tokenAccountingConsistencyRate)} critical={metrics.tokenAccountingConsistencyRate < 1} />
        <Metric label="Tool audit" value={percent(metrics.toolAuditCompletenessRate)} critical={metrics.toolAuditCompletenessRate < 1} />
        <Metric label="Redaction leak" value={percent(metrics.redactionLeakRate)} critical={metrics.redactionLeakRate > 0} />
        <Metric label="Query latency P95" value={`${metrics.queryLatencyP95Ms} ms`} critical={metrics.queryLatencyBudgetPassRate < 1} />
      </section>
      {metrics.releaseGateReasons.length === 0 ? <section className="eval-panel"><strong>Release Gate PASS</strong><p>All deterministic observability gates passed.</p></section> : <section className="eval-panel"><h2>Release Gate FAIL</h2><ul>{metrics.releaseGateReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></section>}
    </>}

    <section className="eval-panel"><h2>Runs</h2><table className="eval-table"><thead><tr><th>Run</th><th>Variant</th><th>Project</th><th>Gate</th><th>Pass</th><th>Open</th></tr></thead><tbody>{snapshot.runs.map((item) => <tr key={item.runId} data-selected={run?.runId === item.runId}><td><code>{item.runId}</code></td><td>{item.variantId}</td><td><code>{item.projectId}</code></td><td>{item.metrics.releaseGate}</td><td>{item.metrics.passedCount}/{item.metrics.sampleCount}</td><td><button type="button" disabled={busy} onClick={() => onSelectRun(item.runId)}>Inspect</button> <a href={traceEvalRunPath(item.runId)}>Result page</a></td></tr>)}</tbody></table></section>

    <section className="eval-panel"><h2>Observability Dataset</h2><table className="eval-table"><thead><tr><th>Scenario</th><th>Split</th><th>Expected</th><th>Result</th></tr></thead><tbody>{snapshot.cases.map((testCase) => { const observation = byCase.get(testCase.caseId); return <tr key={testCase.caseId}><td><strong>{testCase.scenario}</strong><br /><small>{testCase.title}</small></td><td>{testCase.split}</td><td>PASS{testCase.expected.maxLatencyMs === undefined ? '' : ` ≤ ${testCase.expected.maxLatencyMs} ms`}</td><td>{observation === undefined ? '—' : observation.passed ? 'PASS' : 'FAIL'}</td></tr> })}</tbody></table></section>

    <section className="eval-panel"><h2>Failure Drilldown</h2>{snapshot.failures.length === 0 ? <p>No failed trace safety cases in the selected run.</p> : snapshot.failures.map((failure) => {
      ...
      return <article className="eval-panel" key={failure.caseId} data-trace-failure={failure.scenario}>
        <div className="eval-heading-row"><div><strong>{failure.scenario}</strong><p><code>{failure.caseId}</code></p></div><a href={tracePath(failure.traceId)}>Open trace</a></div>
