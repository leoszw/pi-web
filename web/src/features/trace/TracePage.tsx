...
export interface TracePageSnapshot {
...
export function TracePage({
  ...
  initialSnapshot?: TracePageSnapshot
}) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void loadSnapshot(client, traceId)
...
export function TracePageView({
  ...
  return (
    <main className="trace-page" aria-labelledby="trace-title">
      ...
      {tab === 'overview' ? <Overview detail={detail} stats={snapshot.stats} /> : null}
...
function TraceList({ traces }: { traces: readonly TraceSummary[] }) {
  return <main className="trace-page" aria-labelledby="trace-list-title">
    <header className="trace-heading"><div><span>Industry Agent · P5</span><h1 id="trace-list-title">Trace Explorer</h1><p>Project-scoped request traces, audit evidence, token accounting, and debug inspection.</p></div><nav><a href="/industry">Workspace</a><a href="/industry/eval">Evaluation</a></nav></header>
    <section className="trace-panel"><h2>Recent traces</h2><div className="trace-table-wrap"><table className="trace-table"><thead><tr><th>Trace</th><th>Kind</th><th>Status</th><th>Duration</th><th>Spans</th><th>Started</th></tr></thead><tbody>{traces.map((trace) => <tr key={trace.traceId}><td><a href={`/industry/traces/${encodeURIComponent(trace.traceId)}`}>{trace.name}</a><br /><code>{trace.traceId}</code></td><td>{trace.kind}</td><td>{trace.status}</td><td>{trace.durationMs} ms</td><td>{trace.spanCount}</td><td>{trace.startedAt}</td></tr>)}</tbody></table></div></section>
  </main>
}

function Overview({ detail, stats }: { detail: TraceDetail; stats?: TraceStats }) {
  return <>
    <section className="trace-summary-grid" aria-label="Trace stats">
      <Metric label="Duration" value={stats === undefined ? `${detail.summary.durationMs} ms` : `${stats.durationMs} ms`} />
      <Metric label="Query latency" value={stats === undefined ? '—' : `${stats.queryLatencyMs} ms`} />
      <Metric label="LLM calls" value={stats?.llmCallCount ?? '—'} />
      <Metric label="Tool calls" value={stats?.toolCallCount ?? '—'} />
      <Metric label="Input tokens" value={stats?.tokenUsage.inputTokens ?? '—'} />
      <Metric label="Output tokens" value={stats?.tokenUsage.outputTokens ?? '—'} />
      <Metric label="Total tokens" value={stats?.tokenUsage.totalTokens ?? '—'} />
      <Metric label="Errors" value={stats?.errorCount ?? '—'} />
    </section>
    <section className="trace-panel"><h2>Spans</h2><div className="trace-table-wrap"><table className="trace-table"><thead><tr><th>Span</th><th>Kind</th><th>Status</th><th>Duration</th><th>Parent</th></tr></thead><tbody>{detail.spans.map((span) => <tr key={span.spanId}><td><strong>{span.name}</strong><br /><code>{span.spanId}</code></td><td>{span.kind}</td><td>{span.status}</td><td>{span.durationMs} ms</td><td><code>{span.parentSpanId ?? 'root'}</code></td></tr>)}</tbody></table></div></section>
    <section className="trace-panel trace-access"><h2>Granted trace surfaces</h2><p>Debug: <strong>{detail.debug === undefined ? 'NO' : 'YES'}</strong> · Prompt: <strong>{detail.prompts === undefined ? 'NO' : 'YES'}</strong> · Audit: <strong>{detail.audit === undefined ? 'NO' : 'YES'}</strong></p></section>
  </>
}

function Timeline({ events }: { events: readonly TraceTimelineEvent[] }) {
  return <section className="trace-panel"><h2>Timeline</h2>{events.length === 0 ? <p>No timeline events.</p> : <ol className="trace-timeline">{events.map((event) => <li key={event.eventId}><span>#{event.sequenceNo}</span><div><strong>{event.type}</strong><p>{maskTraceText(event.title)}</p>{event.detail === undefined ? null : <small>{maskTraceText(event.detail)}</small>}<small>{event.timestamp}{event.spanId === undefined ? '' : ` · ${event.spanId}`}</small></div></li>)}</ol>}</section>
}

function SpanTree({ tree }: { tree?: TraceTree }) {
  return <section className="trace-panel"><h2>Span Tree</h2>{tree === undefined || tree.roots.length === 0 ? <p>No span tree.</p> : <ul className="trace-tree">{tree.roots.map((node) => <TreeNode key={node.span.spanId} node={node} />)}</ul>}</section>
}

function TreeNode({ node }: { node: TraceTreeNode }) {
  return <li><div><strong>{node.span.name}</strong><span>{node.span.kind} · {node.span.durationMs} ms</span></div>{node.children.length === 0 ? null : <ul>{node.children.map((child) => <TreeNode key={child.span.spanId} node={child} />)}</ul>}</li>
}

function RawDebug({ detail }: { detail: TraceDetail }) {
  return <section className="trace-raw-grid">
    <article className="trace-panel"><h2>Raw / Debug</h2><p>Rendered through a second web-side masking pass.</p><pre>{maskedJson(detail.debug ?? { unavailable: 'trace.read.debug permission required' })}</pre></article>
    <article className="trace-panel"><h2>Prompt</h2>{detail.prompts === undefined ? <p>trace.read.prompt permission required.</p> : <pre>{maskedJson(detail.prompts)}</pre>}</article>
    <article className="trace-panel"><h2>Audit</h2>{detail.audit === undefined ? <p>audit.read permission required.</p> : <pre>{maskedJson(detail.audit)}</pre>}</article>
  </section>
}
...
function tabLabel(tab: TraceTab): string {
  if (tab === 'overview') return 'Overview'
  if (tab === 'timeline') return 'Timeline'
  if (tab === 'tree') return 'Span Tree'
  return 'Raw / Debug'
}
...
async function loadSnapshot(client: IndustryTraceApiClient, traceId: string | undefined): Promise<TracePageSnapshot> {
  ...
  if (traceId === undefined) return { traces }
  const [detail, timeline, tree, stats] = await Promise.all([
    client.getTrace(traceId),
    client.getTimeline(traceId),
    client.getTree(traceId),
    client.getStats(traceId),
  ])
