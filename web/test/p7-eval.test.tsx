...
import type { P7EvalCase, P7EvalDomain, P7EvalMetrics, P7EvalObservation, P7EvalRunSummary } from '../../shared/industry/eval/p7'
...
function snapshot(domain: P7EvalDomain, testCase: P7EvalCase, metrics: P7EvalMetrics, failureStage: NonNullable<P7EvalObservation['failureStage']>): P7EvalSnapshot {
  const selectedRun = run(domain, metrics)
  const observation: P7EvalObservation = { schemaVersion: 'eval-observation-v1', observationId: 'obs-1', runId: selectedRun.runId, caseId: testCase.caseId, domain, title: testCase.title, passed: false, expectedSummary: 'expected', actualSummary: 'actual', failureStage, reasons: ['deterministic failure'], details: {}, traceId: 'trace-p7-project-1-case' }
...
describe('P7EvalView', () => {
  ...
  it('renders all entity failure stages and preserves an 18-digit entity id as a string', () => {
    const id = 'engineering-position-123456789012345678'
    const testCase: P7EvalCase = { schemaVersion: 'eval-case-v1', caseId: 'entity-004', datasetId: 'entity-safety-v1', datasetVersion: '1.0.0', domain: 'ENTITY', title: 'Entity scope', tags: ['entity'], critical: true, mention: '第二合同段路基', expectedEntityId: id, expectedScope: 'project/current' }
