...
describe('AppShell routing', () => {
  ...
  it('merges /industry into the coding chat as the single conversation entry', () => { expect(resolveAppRoute('/industry')).toBe('chat'); const html = renderToStaticMarkup(<AppShell initialPath="/industry" codingChat={<div>Coding chat sentinel</div>} />); expect(html).toContain('Coding chat sentinel'); expect(html).toContain('对话') })
  ...
  it('routes P9 eval labs before the generic eval catch-all', () => {
    expect(resolveAppRoute('/industry/eval/report')).toBe('industry-eval-report'); expect(routePath('industry-eval-report')).toBe('/industry/eval/report'); expect(renderToStaticMarkup(<AppShell initialPath="/industry/eval/report" p9Eval={<div>P9 Eval sentinel</div>} />)).toContain('P9 Eval sentinel')
    expect(resolveAppRoute('/industry/eval/sandbox')).toBe('industry-eval-sandbox'); expect(routePath('industry-eval-sandbox')).toBe('/industry/eval/sandbox'); expect(renderToStaticMarkup(<AppShell initialPath="/industry/eval/sandbox" p9Eval={<div>P9 Eval sentinel</div>} />)).toContain('P9 Eval sentinel')
  })
  ...
  it('routes all four P7 specialist labs before the generic eval catch-all', () => {
    const rows = [['/industry/eval/normalization','industry-eval-normalization'],['/industry/eval/entity','industry-eval-entity'],['/industry/eval/tool','industry-eval-tool'],['/industry/eval/memory','industry-eval-memory']] as const
    for(const[path,route]of rows){expect(resolveAppRoute(path)).toBe(route);expect(routePath(route)).toBe(path);expect(renderToStaticMarkup(<AppShell initialPath={path} p7Eval={<div>P7 Eval sentinel</div>}/>)).toContain('P7 Eval sentinel')}
  })
